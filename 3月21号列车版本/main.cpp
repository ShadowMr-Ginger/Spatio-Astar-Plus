...
void optimize_task(Robot robots[], int start_rearrange_robot_number, list<Cargo>* Cargolist, int Node_movable_node[][4], bool* task_arranged, int node_is_berth[])
{
    ...
    for (int i = 0; i < berth_num; i++) {
        int surplus_berth_capacity = berth[i].cargo_capacity;
...
void Init(int Node_movable_directions[][4], int Node_movable_node[][4], int Berth_information[][5], int Node_is_berth[], int* Boat_capacity, Robot robot_init[])
{
    ...
    */
    ...
    double estimated_rounds_allocate_to_berths[berth_num];
    for (int i = 0; i < berth_num; i++) {
    ...
    while (true){
        ...
        for (int i = 0; i < boat_num; i++) {
            for (int j = 0; j < berth_num;j++) {
                if (!berth[jj].closed &&
                    estimated_rounds_allocate_to_berths[jj] > 0 &&
                    boat[i].surplus_time > berth[jj].round_trip_transport_time) {
                    boat[i].berths_to_go.push_front(jj);
                    estimated_rounds_allocate_to_berths[jj] = estimated_rounds_allocate_to_berths[jj] - 1;
                    boat[i].surplus_time = boat[i].surplus_time - berth[jj].round_trip_transport_time;
                    allocated = true;
                    break;
                }
                else {
